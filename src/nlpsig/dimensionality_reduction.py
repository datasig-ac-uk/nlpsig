...
class DimReduce:
    ...
    ----------
    method : str, optional
        Which dimensionality reduction technique to use, by default "gaussian_random_projection".
        Options are:
    ...
    def __init__(
        self,
        method: str = "gaussian_random_projection",
        n_components: int = 5,
        ...
        if method not in self.implemented_methods:
            raise NotImplementedError(
                ...
                f"{', '.join(self.implemented_methods)}"
            )
    ...
    def fit_transform(self, embeddings: np.array, random_state: int = 42) -> np.array:
        ...
        if self.method == "umap":
            if self.kwargs is None:
                self.kwargs = {"metric": "cosine", "n_neighbors": 50, "min_dist": 0.99}
